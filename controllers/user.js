...
exports.unFriend = async (req, res) => {
  try {
    if (req.user.id != req.params.id) {
      ...
      if (
        reciever.friends.includes(sender._id) &&
        sender.friends.includes(sender._id)
      ) {
        await reciever.updateOne({
          $pull: {
            friends: sender._id,
        ...
        res.json({ message: "unfriend successully" });
      } else {
        return res.status(400).json({ message: "you are not friends yet" });
      }
    } else {
      return res.status(400).json({ message: "You can't sunfriend yourself" });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
  ...
  try {
    if (req.user.id != req.params.id) {
      ...
      if (
        ...
        res.json({ message: "request has been cancelled successfully" });
      } else {
  ...
  try {
    if (req.user.id != req.params.id) {
      ...
      if (reciever.requests.includes(sender._id)) {
        ...
        return res.status(400).json({ message: "You don't have request" });
      }
    } else {
      return res
        .status(400)
        .json({ message: "You can't be delete friend request with yourself" });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
  ...
  try {
    if (req.user.id != req.params.id) {
      ...
      if (
        ...
        res.json({ message: "you are started to following successfully" });
      } else {
...
exports.unfollow = async (req, res) => {
  try {
    if (req.user.id != req.params.id) {
      ...
      const reciever = await User.findById(req.params.id);
      if (
        reciever.follow.includes(sender._id) &&
        sender.following.includes(reciever._id)
      ) {
        await reciever.updateOne({
          $pull: { followers: sender._id },
...
exports.addToSearchHistory = async (req, res) => {
  try {
    ...
    const check = user.search.find(
      (name) => name.user.toString() === searchUser
    );
    if (check) {
      await User.findOne(
        {
